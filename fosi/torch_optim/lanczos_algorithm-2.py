...
def lanczos_alg(order, loss, k_largest, l_smallest=0, return_precision='32', device=None):
    ...
    This function returns a function that performs the Lanczos iterations and can be jitted.e
    Args:
    ...
    def orthogonalization(vecs, cpu_vecs, w, tridiag, i):
        ...
        w1 = (vecs.matmul(w)).matmul(vecs)

        temp_vecs_cpu = vecs.cpu() #将vecs移到内存上，所以内存上需求总是比较高
        
        vecs.copy_(cpu_vecs)

        w2 = (vecs.matmul(w)).matmul(vecs)
        ...
        w1 = (vecs.matmul(w)).matmul(vecs)  # single vector with the shape of w
        vecs.copy_(temp_vecs_cpu)
        w2 = (vecs.matmul(w)).matmul(vecs)
        w = w-(w1+w2)
        ...
        if (i+1) < (order//2):
            ...
            cpu_vecs[i+1-order].copy_((w / beta).squeeze())

        return (tridiag, vecs, cpu_vecs)

    def lanczos_iteration(i, args, params, batch):
        vecs, tridiag, cpu_vecs = args

        # Get last two vectors
        if i < (order//2):
            v = vecs[i]
        else:
            v = cpu_vecs[i-order].cuda()
        # print(v.shape)
        # assert 0
        ...
        if i + 1 < order:
            tridiag, vecs, cpu_vecs = orthogonalization(vecs, cpu_vecs, w, tridiag, i)
    ...
    def lanczos_alg_jitted(params, batch):
        ...
        #################我们只预分配order一半的显存，防止爆存
        tridiag = torch.zeros((order, order), dtype=lanczos_precision).to(device) #这个不大，可以全分配

        assert (order%2) == 0 #这个order必须是2的倍数
        vecs = torch.zeros((order//2, num_params), dtype=lanczos_precision).to(device)
        cpu_vecs = torch.zeros((order//2, num_params), dtype=lanczos_precision) #这一段留在内存上
        # print(cpu_vecs.shape)
        # print(cpu_vecs.device)
        # assert 0
        ...
        for i in range(order):
            ...
            vecs, tridiag, cpu_vecs = lanczos_iter(i, (vecs, tridiag, cpu_vecs))
        ...
        k_largest_eigenvecs = (eigvecs_triag.T[order-k_largest:,:vecs.shape[0]] @ vecs).type(precision)
        l_smallest_eigenvecs = (eigvecs_triag.T[:l_smallest,:vecs.shape[0]] @ vecs).type(precision)
        vecs.copy_(cpu_vecs)
        # print(cpu_vecs.device)
        # assert 0
        k_largest_eigenvecs += (eigvecs_triag.T[order-k_largest:,vecs.shape[0]:] @ vecs).type(precision)
